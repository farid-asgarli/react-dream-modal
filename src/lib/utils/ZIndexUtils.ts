function handler() {
  ...
  return {
    get: getZIndex,
    set: (key: string, el: HTMLElement | null, autoZIndex: boolean, baseZIndex: number) => {
      if (el) {
        el.style.zIndex = String(generateZIndex(key, autoZIndex, baseZIndex));
      }
    },
    clear: (el: HTMLElement | null) => {
      if (el) {
        revertZIndex(ZIndexUtils.get(el));
        el.style.zIndex = "";
      }
    },
    getCurrent: (key: string, autoZIndex: boolean) => getCurrentZIndex(key, autoZIndex),
  };
}
